...
##        fig = plt.figure()
        ...
        gx = makeSubplot(f, 322, 'G(x)','time','deg/s',[-1000,1000], [0,1000])
        gy = makeSubplot(f, 324, 'G(y)','time','deg/s',[-1000,1000], [0,1000])
        gz = makeSubplot(f, 326, 'G(z)','time','deg/s',[-1000,1000], [0,1000])
        bx, = ax.plot([],[])
        by, = ay.plot([],[])
        bz, = az.plot([],[])
        cx, = gx.plot([],[])
...
##################
        anim1 = animation.FuncAnimation(f, theDAQ.refreshplots, fargs=(bx,by,bz,cx,cy,cz,tTFlag), interval=500, blit=FALSE)
##################
