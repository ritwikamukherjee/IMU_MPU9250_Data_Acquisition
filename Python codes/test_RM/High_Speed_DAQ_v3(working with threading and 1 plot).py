...
##                initbuffer()
	...
	def start(self):
		...
		self.acsvwriter.writerow(['AccX', 'AccY', 'AccZ', 'GyrX', 'GyrY', 'GyrZ', 'Temp', 'Arduino Time', 'Computer time'])
...
##                                self.updateBuffer(data[0])
        ...
        def refreshplots(self,zz,bx,by,bz,cx,cy,cz,aflag):
                while (not aflag.is_set()):
                    bx.set_data(self.tbuffer,self.axbuffer)					#bx=(time,ax) by=(time,ay)...
...
def testfunc(aa,aFlag):
    print 'in testfunc'
    print aFlag.is_set()   #learn about aFlag
...
##        fig = plt.figure()
        ax = makeSubplot(f, 321, 'A(x)','time','m/s',[-2000,-1000], [0,1000])
        ay = makeSubplot(f, 323, 'A(y)','time','m/s',[-2000,-1000], [0,1000])
        az = makeSubplot(f, 325, 'A(z)','time','m/s',[-2000,-1000], [0,1000])
        gx = makeSubplot(f, 322, 'G(x)','time','deg/s',[-2000,-1000], [0,1000])
        gy = makeSubplot(f, 324, 'G(y)','time','deg/s',[-1300,-1000], [0,1000])
        gz = makeSubplot(f, 326, 'G(z)','time','deg/s',[-1300,-1000], [0,1000])
...
##################
        anim1 = animation.FuncAnimation(f, theDAQ.refreshplots, fargs=(bx,tTFlag), interval=500, blit=FALSE)
##################
