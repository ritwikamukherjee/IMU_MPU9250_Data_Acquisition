...
##                
	...
	def __init__(self, strPort, bdRate, fname, maxlen):
				...
				self.ao = deque([0.0]*maxLen)
				self.aq = deque([0.0]*maxLen)
				self.am = deque([0.0]*maxLen)
				self.maxLen = maxLen
	...
	def add(self, data):
				...
				self.addToBuf(self.ao, data[7])
				self.addToBuf(self.aq, data[8])
				self.addToBuf(self.am, data[9])
	...
	def start(self):
		...
		self.acsvwriter.writerow(['AccX', 'AccY', 'AccZ', 'GyrX', 'GyrY', 'GyrZ', 'MagX', 'MagY', 'MagZ', 'Temp', 'Time'])
	...
	def writeToCSVFile(self, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9):
                try:
                        first = unpack('!h',self.ser.read(2))
                        if (first[0]==799):
							data = unpack('!hhhhhhh',self.ser.read(20))
							self.acsvwriter.writerow([data[0], data[1], data[2], data[4], data[5], data[6], data[7], data[8], data[9], data[3], time.clock()])  #DATA[3]=TIME
							self.add(data)
							...
							a7.set_data(range(self.maxLen), self.ao)
							a8.set_data(range(self.maxLen), self.aq)
							a9.set_data(range(self.maxLen), self.am)
...
## Step 2 : Figure is being made (not yet included in the GUI object). For more info read: http://matplotlib.org/api/figure_api.html
	...
	ax = makeSubplot(f, 331, 'A(x)','time','m/s',[-15000,15000],[0,100])
	ay = makeSubplot(f, 334, 'A(y)','time','m/s',[-15000,15000],[0,100])
	az = makeSubplot(f, 337, 'A(z)','time','m/s',[-15000,15000],[0,100])
	gx = makeSubplot(f, 332, 'G(x)','time','deg/s',[-15000,15000],[0,100])
	gy = makeSubplot(f, 335, 'G(y)','time','deg/s',[-15000,15000],[0,100])
	gz = makeSubplot(f, 338, 'G(z)','time','deg/s',[-15000,15000],[0,100])
	mx = makeSubplot(f, 333, 'G(z)','time','deg/s',[-15000,15000],[0,100])
	my = makeSubplot(f, 336, 'G(z)','time','deg/s',[-15000,15000],[0,100])
	mz = makeSubplot(f, 339, 'G(z)','time','deg/s',[-15000,15000],[0,100])
...
# intialize two line objects (one in each axes)
	...
	a5, = gy.plot([], [], color='k')
	a6, = gz.plot([], [], color='c')
	a7, = mx.plot([], [], color='#ffa2a7')
	a8, = my.plot([], [], color='#cca2ff ')
	a9, = mz.plot([], [], color='#56ffa2')
	...
	anim1 = animation.FuncAnimation(f, SerialDAQ.writeToCSVFile, 
						 fargs=(a0,a1,a2,a4,a5,a6,a7.a8.a9), interval=500, blit=FALSE)
